    ...
    <section className="relative bg-bg-primary px-6 py-32 md:py-40">
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-16 text-center">
          <p className="mb-3 text-xs font-medium uppercase tracking-[0.2em] text-cobalt">For You</p>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          {audiences.map((item, i) => (
            <div
              key={item.title}
              className="glass group rounded-2xl p-8 transition-all duration-300 hover:bg-white/[0.06] hover:shadow-[0_0_40px_rgba(0,100,173,0.1)]"
              style={{ animation: `fade-up 0.6s ease-out ${0.15 * i}s both` }}
            >
              <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-cobalt/10 text-cobalt transition-all duration-300 group-hover:bg-cobalt group-hover:text-white group-hover:shadow-[0_0_20px_rgba(0,100,173,0.4)]">
                {item.icon}
              </div>
              <div className="mb-3 inline-block rounded-full bg-white/5 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.1em] text-text-muted">
                {item.tag}
              </div>
